...
import { cameraWithTensors, asyncStorageIO, bundleResourceIO} from '@tensorflow/tfjs-react-native';
...
export default function App() {
  ...
  useEffect(() => {
    if (!frameworkReady) {
      (async () => {
        ...
        await tf.ready();
        const tinyBin = require('./assets/model/group1-shard1of1.bin')
        const tinyjson = require('./assets/model/model.json')
        //load the mobilenet model and save it in state
        setMobilenetModel(await loadMobileNetModel());
        //setBlazefaceModel(await tf.loadLayersModel(bundleResourceIO(tinyjson, tinyBin)));
        
        setBlazefaceModel(await loadBlazefaceModel());
  ...
  ------------------------------------------------------------------------*/
  const getPredictionMobile = async (tensor) => {
    if (!tensor) { return; }
    ...
    if (prediction[0].probability > 0.2) {
      ...
      //stop looping!
      cancelAnimationFrame(requestAnimationFrameId);
      setPredictionFound(false);//here true normally
  ...
  const getPredictionBlaze = async (tensor) => {
    if (!tensor) { return; }
    if (blazefaceModel != null) {
    //const prediction = await blazefaceModel.estimateFaces(tensor, false)
  ...
  const handleCameraStream = (imageAsTensors) => {
    const loop = async () => {
      ...
      await getPredictionBlaze(nextImageTensor);
